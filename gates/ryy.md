---
layout: gate
title: $yy$ Rotation
symbol: R_{yy}
alias:
  - YY
notations:
  - R_{yy}(\theta)
  - \mathrm{RYY}(\theta)
  - YY(\theta)
arity: 2
parameters: 1
description: Two-qubit rotation generated by $Y \otimes Y$.
sdks:
  qiskit:
    name: qiskit.circuit.library.RYYGate
    url: https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.RYYGate
  pennylane:
    name: pennylane.IsingYY
    url: https://docs.pennylane.ai/en/stable/code/api/pennylane.IsingYY.html
  cirq:
    name: cirq.YYPowGate
    url: https://quantumai.google/reference/python/cirq/YYPowGate
    note: Uses exponent t where θ = πt, not radians. cirq.YY is the t=1 constant.
---

The $R_{yy}$ gate rotates in the $YY$ interaction basis by angle $\theta$.

$$
\begin{align*}
R_{yy}(\theta) & = \exp\left(-i\frac{\theta}{2}Y\otimes Y\right) \\\\
  & = \begin{bmatrix}
        \cos\frac{\theta}{2}  & 0                      & 0                      & i\sin\frac{\theta}{2} \\\\
        0                     & \cos\frac{\theta}{2}   & -i\sin\frac{\theta}{2} & 0 \\\\
        0                     & -i\sin\frac{\theta}{2} & \cos\frac{\theta}{2}   & 0 \\\\
        i\sin\frac{\theta}{2} & 0                      & 0                      & \cos\frac{\theta}{2}
      \end{bmatrix}
\end{align*}
$$

### Properties

- Entangling for $\theta$ that are not an integer multiple of $\pi$.
- Related to $R_{xx}$ by single-qubit phase rotations: $R_{yy}(\theta) = (S \otimes S)\,R_{xx}(\theta)\,(S^\dagger \otimes S^\dagger)$.
- Generated by a $YY$ interaction Hamiltonian.

### Usage

- Entangling layers in variational algorithms.
- Basis-changed implementations of $XX$ or $ZZ$ couplings.
